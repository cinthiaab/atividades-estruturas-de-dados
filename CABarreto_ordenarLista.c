...
int main()
{
    ...
    printf(" \n\n\nConstruir uma lista com inteiros na faixa 1...1500001 \n\n");
    L = criarListaAleatoria(Max - 1, 1, 1500);
    L1 = L;
    L2 = L;
    L3 = L;
    ...
    finish = clock();
    pf = (double)(finish - start) / CLOCKS_PER_SEC;
    printf("    elapsed = %f segundos ", pf);
    // printf(" \n\n"); mostrarLista(L1);
    printf(" \n\n");

    // printf("Lista Original\n\n"); mostrarLista(L2); printf(" \n\n");
    printf(" executando InsertionSort ... ");
    start = clock();
    ordenarListaCrescenteI(&L2);
    finish = clock();
    pf = (double)(finish - start) / CLOCKS_PER_SEC;
    printf("    elapsed = %f segundos ", pf);
    // printf(" \n\n");mostrarLista(L2);
    ...
    // printf("Lista Original\n\n"); mostrarLista(L3); printf(" \n\n");
    printf(" executando BubbleSort ... ");
    start = clock();
    ordenarListaCrescenteB(&L3);
    finish = clock();
    pf = (double)(finish - start) / CLOCKS_PER_SEC;
    printf("    elapsed = %f segundos ", pf);
    // printf(" \n\n"); mostrarLista(L3);
...
void ordenarListaCrescenteB(Lista *ap)
{ /* BubbleSort: trocar pares consecutivos fora da ordem */
    int i, j, fim, n, aux;
    ...
    { /* n-1 etapas  */
        for (j = 1; j <= fim; j++)
        { /* comparar/trocar do primeiro ao penultimo */
            if (ap->Tabela[j] > ap->Tabela[j + 1])
            {
                aux = ap->Tabela[j];
                ap->Tabela[j] = ap->Tabela[j + 1];
                ap->Tabela[j + 1] = aux;
            };
        }
        fim = fim - 1;
...
void ordenarListaCrescenteS(Lista *ap)
{ /* SelectionSort: localizar o menor e colocar no seu lugar */
    int i, j, min, n, aux;
    n = ap->tamanho; /* n = quantidade de elementos da lista */
    for (i = 1; i < n; i++)
    {            /* n-1 etapas  */
        ...
        for (j = i + 1; j <= n; j++)
            if (ap->Tabela[j] < ap->Tabela[min])
                min = j; /* atualização de min */
        aux = ap->Tabela[i];
        ap->Tabela[i] = ap->Tabela[min];
        ap->Tabela[min] = aux; /* troca com o menor */
    }
}

void ordenarListaCrescenteI(Lista *ap)
{ /* InsertionSort - ordenação por inserção */
    ...
    { /* n-1 etapas  */
        aux = ap->Tabela[i];
        k = i - 1;
        while ((k > 0) && (ap->Tabela[k] > aux))
        {
            ap->Tabela[k + 1] = ap->Tabela[k];
            k = k - 1;
        }                        /* desloca */
